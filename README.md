...
The module provides a `Client` implementation of the low-level protocol. The server implementation is available on the server branch but is pretty useless because device authentication gets in the way. The client still allows you to connect and exchange messages with a Chromecast device without any restriction. 
